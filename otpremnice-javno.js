...
router.get('/:token', async (req, res) => {
  try {
    ...
    const h = await pool.query(
      `SELECT broj, datum, kupac_naziv, kupac_adresa, kupac_grad, komercijalista_ime, status
    ...
    const s = await pool.query(
      `SELECT sifra, naziv, jed_mjera, kolicina, duzina_cm, visina_cm, debljina_cm
    ...
    res.json({
      ...
      stavke: s.rows.map((r, i) => ({
        ...
        duzina_cm: r.duzina_cm, visina_cm: r.visina_cm, debljina_cm: r.debljina_cm,
      })),
    });
  } catch (err) {
