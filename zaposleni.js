  ...
  try {
    const r = await pool.query(
      `SELECT z.id, z.ime_prezime, z.pozicija, z.rola, z.aktivan,
              ...
              z.moze_prodavati, z.moze_roba_magacin, z.moze_cijene, z.komercijalista_teren, z.email,
...
router.patch('/:id', async (req, res) => {
  if (req.session?.user?.rola !== 'admin')
    return res.status(403).json({ error: 'Nema pristupa.' });
 const ALLOWED = ['rola','aktivan','moze_ugovarati','unos_naloga','izmjena_statusa','izmjena_naloga','moze_prodavati','moze_roba_magacin','moze_cijene','komercijalista_teren','email','blagajnik_objekat_id','pozicija','moze_restlovi','moze_restlovi_unos','trening_mod'];
  const sets=[], vals=[];
  let i=1;
